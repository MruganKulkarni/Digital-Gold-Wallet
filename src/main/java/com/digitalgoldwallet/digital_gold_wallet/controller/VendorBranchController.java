...
@Tag(name = "Vendor Branch Management", description = "APIs for managing vendor branches and inventory") // swagger tag groups all branch endpoints
public class VendorBranchController {
    ...
    public VendorBranchController(VendorBranchService vendorBranchService) { // constructor injection
        this.vendorBranchService = vendorBranchService; // assigns injected service
    }
    ...
    @Operation(summary = "Add vendor branch", description = "Adds a new branch to an existing vendor") // swagger description
    public ResponseEntity<VendorBranchResponseDto> addBranch(
            @PathVariable Integer vendorId, // @PathVariable binds {vendorId} from URL
            @Valid @RequestBody VendorBranchRequestDto vendorBranchRequestDto) { // @Valid triggers validation — @RequestBody maps JSON to DTO

        VendorBranchResponseDto response = vendorBranchService.addBranch(vendorId, vendorBranchRequestDto); // calls service to add branch

        return new ResponseEntity<>(response, HttpStatus.CREATED); // returns 201 CREATED with branch response
    }
    ...
    @Operation(summary = "Get branch by ID", description = "Fetches a vendor branch by its unique ID") // swagger description
    public ResponseEntity<VendorBranchResponseDto> getBranchById(
            @PathVariable Integer branchId) { // @PathVariable binds {branchId} from URL

        VendorBranchResponseDto response = vendorBranchService.getBranchById(branchId); // calls service to fetch branch by ID

        return new ResponseEntity<>(response, HttpStatus.OK); // returns 200 OK with branch response
    }
    ...
    @Operation(summary = "Get all branches of a vendor", description = "Fetches all branches belonging to a specific vendor") // swagger description
    public ResponseEntity<List<VendorBranchResponseDto>> getBranchesByVendorId(
            @PathVariable Integer vendorId) { // @PathVariable binds {vendorId} from URL

        List<VendorBranchResponseDto> response = vendorBranchService.getBranchesByVendorId(vendorId); // calls service to fetch all branches of vendor

        return new ResponseEntity<>(response, HttpStatus.OK); // returns 200 OK with list of branches
    }
    ...
    @Operation(summary = "Get branch inventory", description = "Fetches the current gold inventory quantity at a specific branch") // swagger description
    public ResponseEntity<BigDecimal> getInventoryByBranchId(
            @PathVariable Integer branchId) { // @PathVariable binds {branchId} from URL

        BigDecimal inventory = vendorBranchService.getInventoryByBranchId(branchId); // calls service to get inventory

        return new ResponseEntity<>(inventory, HttpStatus.OK); // returns 200 OK with inventory quantity
    }
}