...
import jakarta.validation.constraints.Min; // validates minimum value on path variable
...
import org.springframework.validation.annotation.Validated; // enables method-level validation
...
@Validated // enables method-level validation for @Min on path variables
...
public class VendorController {
    ...
    public ResponseEntity<VendorResponseDto> getVendorById(
            @PathVariable @Min(value = 1, message = "Vendor ID must be positive") Integer vendorId) { // @PathVariable binds {vendorId} from URL
    ...
    /*
     * GET /api/v1/vendors
     * Fetches ALL vendors — no parameters needed
     * Returns 200 OK with complete list of vendors
     */
    @GetMapping // maps GET /api/v1/vendors
    @Operation(summary = "Get all vendors", description = "Fetches all gold vendors") // swagger description
    public ResponseEntity<List<VendorResponseDto>> getAllVendors() { // no parameters — returns all vendors

        List<VendorResponseDto> response = vendorService.getAllVendors(); // calls service to fetch all vendors

        return new ResponseEntity<>(response, HttpStatus.OK); // returns 200 OK with all vendors
    }
